...
function Header() {
    return (
        <div className="w-full h-max p-6 bg-blue-200 flex justify-between">
            ...
            <div>
            <div
            className="group relative inline-block rounded-full bg-indigo-600 px-8 py-3 text-white"
            >
    ...
    return (
        <div className="h-full flex flex-col p-4">
           {sidebarItems.map((itm, index) => {
            return (
                <div key={index} className="p-4 rounded-md text-2xl"><Link href={itm.to}>{itm.title}</Link></div>
            )
           })}
        </div>
    )
...
function Footer() {
    return (
        <div className="w-screen h-full p-4 font-bold flex justify-between">
            <div>Anime Tracker 2023</div>
            <div>Socials</div>
        </div>
    )
}
